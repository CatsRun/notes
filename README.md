...
4.	In the terminal: 
    ...
    c.	npx tsc nameOfFile.ts
            1.	typescript compile extension tool. This will run the typescript through  this and convert it into JavaScript.
            2.	When I	t is converted back to JS it verifies that the types are correct
        ii.	A new file is made with the same file name but .js instead of .ts
        iii.	To run in terminal: 
            1.	Node filename.js
    d.	npx tsc –init
        i.	creates tsconfig.json
            1.	this is where config can be edited
